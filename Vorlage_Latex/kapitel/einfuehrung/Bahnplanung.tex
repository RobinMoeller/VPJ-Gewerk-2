...
Beispielsweise könnte bei der Aufgabenstellung \glqq Finde den schnellsten Weg zum Ziel\grqq ein anderer Berechnungsansatz der Bahnplanung verfolgt werden, als bei \glqq Finde den energieeffizientesten Weg zum Ziel\grqq. Weitere Kriterien zum Auswählen eines Bahnplanungsansatzes könnten sein, die kürzeste Strecke zu finden oder Strecken nach Vorhersagbarkeit des Weges zu bewerten.
Allgemein wird in der Bahnplanung zwischen globaler und lokaler Bahnplanung unterschieden. 
...
Die Eckpunkte der Hindernisse werden dann durch eine Kante verbunden, wenn eine gerade Verbindungslinie gezogen werden kann ohne andere Hindernisse zu schneiden. Dadurch werden Eckpunkte zu Knotenpunkten. Die Hindernisse mit den Verbindungslinien ist in Abbildung \ref{fig:Sichtbarkeitsgraph} dargestellt. 

\begin{figure}[h]
	\centering	
	\includegraphics[width=12cm]{Sichtbarkeitsgraph.png}
...
\begin{figure}[h]
	\centering	
	\includegraphics[width=10cm]{OccupancyGrid.png}
...
\begin{figure}[h]
	\centering	
	\includegraphics[width=10cm]{DStern.png}
...
Der \glqq Bug-Algorithmus \grqq{} ist im Allgemeinen eine reaktive Navigationsmethode und basiert auf dem Prinzip, dass sich der Roboter sich so lange entlang eines Hindernisses bewegt, bis er wieder freie Fahrt in Richtung Ziel hat. Es gibt verschiedene Algorithmen, die diese Methode verfolgen. Dazu gehört der \glqq Bug1 -\grqq, \glqq Bug2-\grqq, \glqq Bug3-\grqq und \glqq DistBug-Algorithmus \grqq{}. Im Folgenden wird nur der Bug2-Algorithmus näher gehend erläutert. 
...
\begin{figure}[h]
	\centering	
	\includegraphics[width=5cm]{Bug2_m-Linie.png}
...
\begin{figure}[h]
	\centering	
	\includegraphics[width=5cm]{Bug2.png}
...
\begin{figure}[h]
	\centering	
	\includegraphics[width=5cm]{Bug2_Problem.png}
...
Im Folgenden wird das Gesamtkonzept mit der Bahnplanungsmethode Potentialfeld kurz erläutert. Genauere Erklärungen mit Auszügen aus dem Originalprogramm wird anschließend erläutert. 
\begin{figure}[h!]
	\centering	
	\includegraphics[width=11cm]{KonfigRaum.png}
	\caption{Konfigurationsraum}
...
\subsubsection{Warteporitionen}
Jedem Robotino wird eine eigene Warteposition am Rand des Konfigurationsraumes zugeordnet. Diese Warteposition ist zu Beginn des Gesamtsystem die Startposition des jeweiligen Robotinos und immer dann Zielposition, wenn der Robotino keinen Auftrag empfängt. Sollten alle \glqq Fifo-Plätz\grqq{} einer Station belegt sein, so dass sich ein Robotino nicht mehr anstellen kann, muss er aus seiner Warteposition auf einen freien Platz warten. 

\subsubsection{Stationen und Übergabepunkte}
Wie in Abbildung \ref{fig:Konfigurationsraum} zu sehen ist, befindet sich vor und hinter jeder Station Übergabepunkte. Hat ein Robotino die Aufgabe zu einer Station zu fahren, fährt er stattdessen nur vor die Station auf den Übergabepunkt. Ab da wird Steuerung des Roboters an das Gewerk3 \glqq Regelung\grqq{} übergeben. Sollten mehrere Robotinos die gleiche Station anfahren wollen, so darf der Robotino die Station zuerst befahren, der den Übergabepunkt zuerst erreicht hat. Über die Übergabepunkte wird die Station auch wieder verlassen. Der vordere Übergabepunkt wird immer dann genutzt, wenn sich kein weitere Robotino im Fifo befindet. Über den hinteren Übergabepunkt wird die Station verlassen, wenn ein andere Robotino bereits im Fifo wartet. 
Das Potentialfeld ist in den Stationen, solange Gewerk3 \glqq Regelung\grqq{} den Robotino steuert, nicht aktiv. Aktiviert bzw. ausgeschaltet wird das Potentialfeld zum Zeitpunkt der Übergabe. Sollte ein Robotino in eine Station gedrückt werden, so verlässt er sie unverzüglich nach hinten raus. In diesem Fall bleibt das Potentialfeld aktiv. 

\subsubsection{Warteschlange \glqq Fifo\grqq}
Ist eine Station, in die ein Robotino fahren soll, bereits belegt, so fährt er zu der zugehörigen Warteschlange, dem sogannten \glqq Fifos\grqq{}. Die Fifos sind Wartepositionen für die Stationen, die sich am Rand des Konfigurationsraumes befinden, um während des Wartens den befahrbaren Raum nicht zu blockieren. Der Robotino, der die Warteschlange zuerst betreten hat, darf diese, sobald die Station wieder frei ist, auch als erster wieder verlassen. Erst, wenn dieser die Station erreicht hat, rücken eventuell wartende Robotinos im gleichen Fifo auf. 
...
\begin{figure}[h!]
	\centering	
	\includegraphics[width=11cm]{Potentialfeld_Fertig.png}
...
Zur Vermeidung von kritischen Situartionen oder lokalen Minima zwischen Stationen und Wand sind Rampen und Fahrrinnen implementiert worden, die je nach Station den Roboter nach links oder rechts in eine Fahrrinne und von dort in den frei befahrbaren Raum führen. 