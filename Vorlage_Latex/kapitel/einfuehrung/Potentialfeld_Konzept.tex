...
Um die statische Umwelt des Robotinos im Potentialfeld darzustellen, wird der zu befahrene Bereich in Zonen unterteilt. Die gewählten Zonen sind in Abbildung \ref{fig:PotZonen} farbig dargestellt. In Tabelle \ref{tab:Zonen} ist die Zuordnung der Zonen zur Abbildung \ref{fig:PotZonen} beschrieben.\\
 \begin{table}[H]
\centering
 \begin{tabular}{|c|c|c|}
...
Aufgrund der Form der Zone müssen dazu drei Funktionen definiert werden, die je nach Position des Robotinos ausgeführt werden. Das Verlassen der Zone 0 erfolgt durch die Übergabe an den Fertigungsbereich, welches in Kapitel \ref{sec:Kon_Bahnregelung} näher erläutert wird. \\
Zone 1 bis 3 dienen zur Rückführung der Robotinos zur Zone 0, dazu wird ein Potentialfeld genutzt, dass es ermöglicht den Robotino in eine gezielte Richtung zu lenken. Die dazu genutzten Funktionen sind unter \eqref{fun:H1} bis \eqref{fun:H3} definiert. Diese Potentialfelder nutzen in Fahrrichtung eine Geradengleichung mit definierter Steigung und senkrecht dazu eine Parabelform, um den Robotino auf Kurs zu halten. Wenn sich der Robotino in Zone 4 befindet, wird dieser über eine Rampe nach hinten geführt. Die dabei genutzte Gleichung ist als \eqref{fun:H4} gekennzeichnet. Zusätzlich wird in Zone 4 für jede Station eine gaußsche radiale Basisfunktion, welche unter Kapitel \ref{sec:PotRob} näher erläutert wird, genutzt, damit keine Kollision mit den Stationen auftreten. Diese Zone wird nur im Fehlerfall oder beim Start des Robotinos betreten, da in dieser Zone der Fertigungsbereich aktiv ist, in der die Bahnregelung die Steuerung des Robotinos übernimmt. Die Zone 5 dient dazu den Robotino gezielt in Richtung der Mitte der Zone 0 zu führen. Dazu wird, wie in Zone 1 bis 3, eine Parabel mit einer Geradengleichung genutzt. Die dabei genutzte Funktion ist als \eqref{fun:H5} definiert. Dabei ist zu beachten, dass vorgesehen ist, dass Zone 5 nur aktiv ist, wenn sie aus Zone 1 betreten wird. Dadurch wird die Anfahrmöglichkeit aus der Zone 0 zur Ladestation gewährleistet. \\
...
Das Ziel im Potentialfeld wird als Kegel mit konstantem Faktor realisiert. Dazu wird die in Gleichung \eqref{fun:H_Ziel} dargestellte Funktion genutzt. Um durch Trägheit verursachte Schwingungen um den Zielpunkt zu vermeiden, wird ein abstandsabhängiger Faktor als Filterung nachgeschaltet. Das Zielpotentialfeld ist in Abbildung \ref{fig:PotZiel} dargestellt.
